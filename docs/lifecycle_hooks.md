...
2. `handle_before_normalize`
3. `handle_after_normalize`
4. `handle_before_record`
5. `handle_after_record`
6. `handle_before_response`

Notes:
- `record` mode reuses normalize request/response handling, so normalize hooks run before record hooks.
...
- With `test_policy=none`, normalize/test hooks are skipped and the flow is handled by mock logic.
