...
from stoobly_agent.config.constants import env_vars, mode, mock_policy, develop_policy, record_order, record_policy, record_strategy, test_policy, test_strategy
...
class TestInterceptSet():
  ...
  class TestRecordOrder():
    ...
    class TestScenario():
      ...
      @pytest.fixture(scope='class', autouse=True)
      def set(self, runner: CliRunner, scenario: Scenario):
        set_result = runner.invoke(setting, ['scenario', 'set', scenario.key()])
        assert set_result.exit_code == 0

        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD ,'--order', record_order.OVERWRITE])
        assert set_result.exit_code == 0
      ...
      class TestChangingOrder():
        ...
        def test_scenario_not_overwritable(self, runner: CliRunner, scenario: Scenario):
          ...
          set_result = runner.invoke(intercept, ['set', '--order', record_order.APPEND])
          assert set_result.exit_code == 0
  ...
  class TestPolicy():
    ...
    class TestMockPolicy():
      ...
      def test_policy_mock_mode_all(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.MOCK, '--policy', mock_policy.ALL])
        assert set_result.exit_code == 0
      ...
      def test_policy_mock_mode_found(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.MOCK, '--policy', mock_policy.FOUND])
        assert set_result.exit_code == 0
      ...
      def test_policy_without_mode_mock_existing(self, runner: CliRunner):
        ...
        set_result = runner.invoke(intercept, ['set', '--policy', mock_policy.ALL])
        assert set_result.exit_code == 0
    ...
    class TestRecordPolicy():
      ...
      def test_policy_record_mode_all(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD, '--policy', record_policy.ALL])
        assert set_result.exit_code == 0
      ...
      def test_policy_record_mode_api(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD, '--policy', record_policy.API])
        assert set_result.exit_code == 0
      ...
      def test_policy_record_mode_found(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD, '--policy', record_policy.FOUND])
        assert set_result.exit_code == 0
      ...
      def test_policy_record_mode_not_found(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD, '--policy', record_policy.NOT_FOUND])
        assert set_result.exit_code == 0
      ...
      def test_policy_without_mode_record_existing(self, runner: CliRunner):
        ...
        set_result = runner.invoke(intercept, ['set', '--policy', record_policy.API])
        assert set_result.exit_code == 0
    ...
    class TestReplayPolicy():
      ...
      def test_policy_replay_mode_all(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.DEVELOP, '--policy', develop_policy.ALL])
        assert set_result.exit_code == 0

        settings = Settings.instance()
        project_key = ProjectKey(settings.proxy.intercept.project_key)
        data_rule = settings.proxy.data.data_rules(project_key.id)
        assert data_rule.develop_policy == develop_policy.ALL

      def test_policy_without_mode_replay_existing(self, runner: CliRunner):
        runner.invoke(intercept, ['set', '--mode', mode.DEVELOP])
        set_result = runner.invoke(intercept, ['set', '--policy', develop_policy.ALL])
        assert set_result.exit_code == 0
    ...
    class TestInvalidPolicyInput():
      ...
      def test_policy_invalid_for_mock_mode(self, runner: CliRunner):
        ...
        set_result = runner.invoke(intercept, ['set', '--mode', mode.MOCK, '--policy', record_policy.API])
      ...
      def test_policy_invalid_for_replay_mode(self, runner: CliRunner):
        # Use record_policy.API which is not valid for DEVELOP mode
        set_result = runner.invoke(intercept, ['set', '--mode', mode.DEVELOP, '--policy', record_policy.API])
        assert set_result.exit_code == 1
        assert "Error: Valid policies for" in set_result.output
      ...
      def test_policy_invalid_for_test_mode(self, runner: CliRunner, intercept_cli: Group):
        ...
        set_result = runner.invoke(intercept_cli, ['set', '--mode', mode.TEST, '--policy', record_policy.API])
        assert set_result.exit_code == 1
        assert "Error: Valid policies for" in set_result.output
  ...
  class TestStrategy():
    ...
    class TestRecordStrategy():
      ...
      def test_strategy_record_mode_full(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD, '--strategy', record_strategy.FULL])
        assert set_result.exit_code == 0
      ...
      def test_strategy_record_mode_minimal(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.RECORD, '--strategy', record_strategy.MINIMAL])
        assert set_result.exit_code == 0
      ...
      def test_strategy_without_mode_record_existing(self, runner: CliRunner):
        runner.invoke(intercept, ['set', '--mode', mode.RECORD])
        set_result = runner.invoke(intercept, ['set', '--strategy', record_strategy.MINIMAL])
        assert set_result.exit_code == 0
    ...
    class TestTestStrategy():
      ...
      def test_strategy_test_mode_contract(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.TEST, '--strategy', test_strategy.CONTRACT])
        assert set_result.exit_code == 0
      ...
      def test_strategy_test_mode_diff(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.TEST, '--strategy', test_strategy.DIFF])
        assert set_result.exit_code == 0
      ...
      def test_strategy_without_mode_test_existing(self, runner: CliRunner):
        runner.invoke(intercept, ['set', '--mode', mode.TEST])
        set_result = runner.invoke(intercept, ['set', '--strategy', test_strategy.FUZZY])
        assert set_result.exit_code == 0
    ...
    class TestInvalidInput():
      def test_strategy_mock_mode_error(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.MOCK, '--strategy', record_strategy.FULL])
        assert set_result.exit_code == 1
      ...
      def test_strategy_replay_mode_error(self, runner: CliRunner):
        set_result = runner.invoke(intercept, ['set', '--mode', mode.DEVELOP, '--strategy', record_strategy.FULL])
        assert set_result.exit_code == 1
        assert "Error: set --strategy to a intercept mode that supports the strategy option" in set_result.output

      def test_strategy_without_mode_unsupported_existing(self, runner: CliRunner):
        runner.invoke(intercept, ['set', '--mode', mode.MOCK])
        set_result = runner.invoke(intercept, ['set', '--strategy', record_strategy.FULL])
        assert set_result.exit_code == 1
        assert "Error: set --strategy to a intercept mode that supports the strategy option" in set_result.output
