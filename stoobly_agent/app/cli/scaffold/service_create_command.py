...
from .app import App
from .constants import WORKFLOW_DEVELOP_TYPE, WORKFLOW_MOCK_TYPE, WORKFLOW_RECORD_TYPE, WORKFLOW_TEST_TYPE
...
class ServiceCreateCommand(ServiceCommand):
  ...
  def build(self):
    ...
    if WORKFLOW_DEVELOP_TYPE in self.workflows:
      self.__build_with_develop_workflow(service_builder, **workflow_kwargs)
  ...
  def __build_with_develop_workflow(self, service_builder: Union[ServiceBuilder, DockerServiceBuilder], **kwargs):
    develop_workflow = WorkflowCreateCommand(self.app, **{ **kwargs, **{ 'workflow_name': WORKFLOW_DEVELOP_TYPE }})

    workflow_decorators = get_workflow_decorators(WORKFLOW_DEVELOP_TYPE, self.service_config)
    develop_workflow.build(service_builder=service_builder, workflow_decorators=workflow_decorators)
