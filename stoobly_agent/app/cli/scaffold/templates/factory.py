...
from .constants import (
  ...
  DEVELOP_WORKFLOW_CUSTOM_FILES, DEVELOP_WORKFLOW_CUSTOM_LOCAL_FILES, DEVELOP_WORKFLOW_MAINTAINED_FILES, DEVELOP_WORKFLOW_CUSTOM_DOCKER_FILES,
...
def custom_files(workflow: str, workflow_builder: WorkflowBuilder = None):
  ...
  elif workflow == WORKFLOW_DEVELOP_TYPE:
    files = DEVELOP_WORKFLOW_CUSTOM_FILES.copy()
  ...
  if isinstance(workflow_builder, DockerWorkflowBuilder):
    if workflow == WORKFLOW_MOCK_TYPE:
      ...
      files.extend(DEVELOP_WORKFLOW_CUSTOM_DOCKER_FILES)
    elif workflow == WORKFLOW_RECORD_TYPE:
    ...
    if workflow == WORKFLOW_MOCK_TYPE:
      ...
      files.extend(DEVELOP_WORKFLOW_CUSTOM_LOCAL_FILES)
    elif workflow == WORKFLOW_RECORD_TYPE:
      files.extend(RECORD_WORKFLOW_CUSTOM_LOCAL_FILES)
    elif workflow == WORKFLOW_TEST_TYPE:
      files.extend(TEST_WORKFLOW_CUSTOM_LOCAL_FILES)

  # Fixtures are only relevant if the workflow is mock/test/develop and if the service has a hostname
...
def maintained_files(workflow: str, workflow_builder: WorkflowBuilder = None):
  ...
  if workflow == WORKFLOW_MOCK_TYPE:
    ...
    files = DEVELOP_WORKFLOW_MAINTAINED_FILES.copy()
  elif workflow == WORKFLOW_RECORD_TYPE:
