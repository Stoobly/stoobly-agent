import os

from typing import Literal

from stoobly_agent.config.data_dir import CERTS_DIR_NAME, DATA_DIR_NAME

APP_DIR = '${APP_DIR}'
APP_DIR_ENV = 'APP_DIR'
APP_DIR_MOUNT = '${APP_DIR_MOUNT}'
APP_DIR_MOUNT_ENV = 'APP_DIR_MOUNT'
APP_COPY_ON_WORKFLOW_UP_ENV = 'APP_COPY_ON_WORKFLOW_UP'
APP_DOCKER_SOCKET_PATH_ENV = 'APP_DOCKER_SOCKET_PATH'
APP_NETWORK_ENV = 'APP_NETWORK'
APP_NAME_ENV = 'APP_NAME'
APP_PLUGINS_ENV = 'APP_PLUGINS'
APP_PLUGINS_DELMITTER = ','
APP_PROXY_HOSTNAME_ENV = 'APP_PROXY_HOSTNAME'
APP_PROXY_MODE_ENV = 'APP_PROXY_MODE'
APP_PROXY_PORT_ENV = 'APP_PROXY_PORT'
APP_RUNTIME_ENV = 'APP_RUNTIME'
APP_UI_PORT_ENV = 'APP_UI_PORT'
APP_VERSION_ENV = 'APP_VERSION'
BIN_FOLDER_NAME = 'bin'
CA_CERTS_DIR_ENV = 'CA_CERTS_DIR'
CERTS_DIR_ENV = 'CERTS_DIR'
CONFIG_FILE = '.config.yml'
CONTEXT_DIR_ENV = 'CONTEXT_DIR'
CONTEXT_DIR_MOUNT = '${CONTEXT_DIR_MOUNT}'
CONTEXT_DIR_MOUNT_ENV = 'CONTEXT_DIR_MOUNT'
DOTENV_FILE = '.env'
DOTENV_FILE_ENV = 'STOOBLY_DOTENV_FILE'
FIXTURES_FILE_NAME = 'fixtures.yml'
OPENAPI_SPECIFICATION_FILE_NAME = 'openapi.yml'
NAMESERVERS_FILE = '.nameservers'
PLUGIN_CYPRESS = 'cypress'
PLUGIN_PLAYWRIGHT = 'playwright'
PLUGINS_FOLDER = 'plugins'
PROXY_MODE_FORWARD = 'forward'
PROXY_MODE_REVERSE = 'reverse'
PUBLIC_FOLDER_NAME = 'public'
RUNTIME_DOCKER = 'docker'
RUNTIME_LOCAL = 'local'
RUNTIME_APP_DIR = '${RUNTIME_APP_DIR}'
RUNTIME_APP_DIR_ENV = 'RUNTIME_APP_DIR'
SERVICE_DETACHED = '${SERVICE_DETACHED}'
SERVICE_DETACHED_ENV = 'SERVICE_DETACHED'
SERVICE_DNS = '${SERVICE_DNS}'
SERVICE_DNS_ENV = 'SERVICE_DNS'
SERVICE_HOSTNAME = '${SERVICE_HOSTNAME}'
SERVICE_HOSTNAME_ENV = 'SERVICE_HOSTNAME'
SERVICE_ID = '${SERVICE_ID}'
SERVICE_ID_ENV = 'SERVICE_ID'
SERVICE_LOCAL_ENV = 'SERVICE_LOCAL'
SERVICE_NAME = '${SERVICE_NAME}'
SERVICE_NAME_ENV = 'SERVICE_NAME'
SERVICE_OPENAPI_SPECIFICATION_ENV = 'SERVICE_OPENAPI_SPECIFICATION'
SERVICE_SCHEME = '${SERVICE_SCHEME}'
SERVICE_SCHEME_ENV = 'SERVICE_SCHEME'
SERVICE_PORT = '${SERVICE_PORT}'
SERVICE_PORT_ENV = 'SERVICE_PORT'
SERVICE_PRIORITY_ENV = 'SERVICE_PRIORITY'
SERVICE_SCRIPTS = '${SERVICE_SCRIPTS}'
SERVICE_SCRIPTS_DIR = '/usr/local/bin/services'
SERVICE_SCRIPTS_ENV = 'SERVICE_SCRIPTS'
SERVICE_UPSTREAM_HOSTNAME = '${SERVICE_UPSTREAM_HOSTNAME}'
SERVICE_UPSTREAM_HOSTNAME_ENV = 'SERVICE_UPSTREAM_HOSTNAME'
SERVICE_UPSTREAM_PORT = '${SERVICE_UPSTREAM_PORT}'
SERVICE_UPSTREAM_PORT_ENV = 'SERVICE_UPSTREAM_PORT'
SERVICE_UPSTREAM_SCHEME = '${SERVICE_UPSTREAM_SCHEME}'
SERVICE_UPSTREAM_SCHEME_ENV = 'SERVICE_UPSTREAM_SCHEME'
SERVICES_NAMESPACE = 'services'
STOOBLY_HOME_DIR = '/home/stoobly'
STOOBLY_DATA_DIR = os.path.join(STOOBLY_HOME_DIR, DATA_DIR_NAME)
STOOBLY_CERTS_DIR = os.path.join(STOOBLY_DATA_DIR, CERTS_DIR_NAME)
USER_ID_ENV = 'USER_ID'
WORKFLOW_ACCESS_COUNT_ENV = 'WORKFLOW_ACCESS_COUNT'
WORKFLOW_CONTAINER_BRIDGE = 'bridge'
WORKFLOW_CONTAINER_INIT = 'init'
WORKFLOW_CONTAINER_PROXY = 'proxy'
WORKFLOW_CONTAINER_SERVICE = 'service'
WORKFLOW_CONTAINER_TEMPLATE = '{service_name}.{container}'
WORKFLOW_CONTAINER_INIT_TEMPLATE = '{service_name}.' + WORKFLOW_CONTAINER_INIT
WORKFLOW_CONTAINER_PROXY_TEMPLATE = '{service_name}.' + WORKFLOW_CONTAINER_PROXY
WORKFLOW_DEVELOP_TYPE = 'develop'
WORKFLOW_EXEC_TYPE = 'exec'
WORKFLOW_MOCK_TYPE = 'mock'
WORKFLOW_NAME = '${WORKFLOW_NAME}'
WORKFLOW_NAME_ENV = 'WORKFLOW_NAME'
WORKFLOW_NAMESPACE = '${WORKFLOW_NAMESPACE}'
WORKFLOW_NAMESPACE_ENV = 'WORKFLOW_NAMESPACE'
WORKFLOW_RECORD_TYPE = 'record'
WORKFLOW_SCRIPTS = '${WORKFLOW_SCRIPTS}'
WORKFLOW_SCRIPTS_DIR = '/usr/local/bin/workflows'
WORKFLOW_SCRIPTS_ENV = 'WORKFLOW_SCRIPTS'
WORKFLOW_TEMPLATE = '${WORKFLOW_TEMPLATE}'
WORKFLOW_TEMPLATE_ENV = 'WORKFLOW_TEMPLATE'
WORKFLOW_TEST_TYPE = 'test'

CORE_WORKFLOWS = [WORKFLOW_DEVELOP_TYPE, WORKFLOW_MOCK_TYPE, WORKFLOW_RECORD_TYPE, WORKFLOW_TEST_TYPE]
WORKFLOW_TEMPLATE_OPTION = Literal[WORKFLOW_DEVELOP_TYPE, WORKFLOW_MOCK_TYPE, WORKFLOW_RECORD_TYPE, WORKFLOW_TEST_TYPE]
RUNTIME_OPTIONS = [RUNTIME_DOCKER, RUNTIME_LOCAL]
