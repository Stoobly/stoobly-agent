AGENT_CONTEXT_DIR = 'STOOBLY_AGENT_CONTEXT_DIR' # Only set by the run command 
AGENT_ENABLED = 'STOOBLY_AGENT_ENABLED'
AGENT_SELF_INTERCEPT_ENABLED = 'STOOBLY_AGENT_SELF_INTERCEPT_ENABLED'
AGENT_HEADLESS = 'STOOBLY_AGENT_HEADLESS'
AGENT_INCLUDE_PATTERNS = 'STOOBLY_AGENT_INCLUDE_PATTERNS'
AGENT_INTERCEPT_ACTIVE = 'STOOBLY_AGENT_INTERCEPT_ACTIVE'
AGENT_INTERCEPT_MODE = 'STOOBLY_AGENT_INTERCEPT_MODE'
AGENT_EXCLUDE_PATTERNS = 'STOOBLY_AGENT_EXCLUDE_PATTERNS'
AGENT_LIFECYCLE_HOOKS_PATH = 'STOOBLY_AGENT_LIFECYCLE_HOOKS_PATH'
AGENT_OPENAPI_SPECIFICATION_PATH = 'STOOBLY_AGENT_OPENAPI_SPECIFICATION_PATH'
AGENT_PROJECT_KEY = 'STOOBLY_AGENT_PROJECT_KEY'
AGENT_PROXY_URL = 'STOOBLY_AGENT_PROXY_URL'
AGENT_PUBLIC_DIRECTORY_PATH = 'STOOBLY_AGENT_PUBLIC_DIRECTORY_PATH'
AGENT_REMOTE_ENABLED = 'STOOBLY_AGENT_REMOTE_ENABLED'
AGENT_REMOTE_PROJECT_KEY = 'STOOBLY_AGENT_REMOTE_PROJECT_KEY'
AGENT_REPLAY_HOST = 'STOOBLY_AGENT_REPLAY_HOST'
AGENT_REPLAY_SCHEME = 'STOOBLY_AGENT_REPLAY_SCHEME'
AGENT_RESPONSE_FIXTURES_PATH = 'STOOBLY_AGENT_RESPONSE_FIXTURES_PATH'
AGENT_SCENARIO_KEY = 'STOOBLY_AGENT_SCENARIO_KEY'
AGENT_SERVICE_URL = 'STOOBLY_AGENT_SERVICE_URL'
AGENT_SETTINGS_PATH = 'STOOBLY_AGENT_SETTINGS_PATH'
AGENT_SIMULATE_LATENCY = 'STOOBLY_AGENT_SIMULATE_LATENCY'
AGENT_UI_URL = 'STOOBLY_AGENT_UI_URL'
API_URL = 'STOOBLY_API_URL'
API_KEY = 'STOOBLY_API_KEY'
APP_DIR = 'STOOBLY_APP_DIR'
CA_CERTS_DIR = 'STOOBLY_CA_CERTS_DIR'
CERTS_DIR = 'STOOBLY_CERTS_DIR'
CONTEXT_DIR = 'STOOBLY_CONTEXT_DIR' # Only set by the user explicitly
ENV = 'STOOBLY_AGENT_ENV'
FEATURE_REMOTE = 'STOOBLY_FETURE_REMOTE'
HTTP_PROXY = 'HTTP_PROXY'
HTTPS_PROXY = 'HTTPS_PROXY'
LOG_LEVEL = 'STOOBLY_AGENT_LOG_LEVEL'
NO_PROXY = 'NO_PROXY'